...
function inviscid_rhs_el!(u, params, connijk, qe, x, y, lsource, SD::NSD_2D)
    ...
    for iel = 1:params.mesh.nelem
        ...
        De,be = compute_matirx(params,connijk,iel,x,y)

        coef = compute_coef(params,De,be)
        ...
        #=  user_flux!(@view(params.F[k,l,:]), @view(params.G[k,l,:]), SD,
                ...
                #=user_flux!(@view(params.F[k,l,:]), @view(params.G[k,l,:]), SD,
                       params,connijk,iel,k,l,x,y,
                      @view(qe[ip,:]),         #pref
                      params.mesh,
                      params.CL, params.SOL_VARS_TYPE;
                      neqs=params.neqs, ip=ip) # modified=#
                     ...
                     user_flux!(@view(params.F[k,l,:]), @view(params.G[k,l,:]), SD,
                      ...
                      params.mesh,
                      params.CL, params.SOL_VARS_TYPE;
                      neqs=params.neqs, ip=ip) #new faster
            ...
            user_primitives!(@view(params.uaux[ip,:]), @view(qe[ip,:]), @view(params.uprimitive[k,l,:]), params.SOL_VARS_TYPE)
                        ...
                        for j = 1:params.mesh.ngl
                            for i = 1:params.mesh.ngl
                                ...
                                ψJK = ψ[i,k]*ψ1[j,l]
                                dψJK_dx = dψ[i,k]*ψ1[j,l]*dξdx[iel,k,l] + ψ[i,k]*dψ1[j,l]*dηdx[iel,k,l]
                                dψJK_dy = dψ[i,k]*ψ1[j,l]*dξdy[iel,k,l] + ψ[i,k]*dψ1[j,l]*dηdy[iel,k,l]

                                be[K,:] += ωkl*Jkle*@view(params.uaux[ip,:])*ψJK

                                for n = 1:params.mesh.ngl
                                    for m = 1:params.mesh.ngl

                                        I = m + (n - 1)*(params.mesh.ngl)#ψIK = ψ[m,k]*ψ1[n,l]
                                        dψIK_dx = dψ[m,k]*ψ1[n,l]*dξdx[iel,k,l] + ψ[m,k]*dψ1[n,l]*dηdx[iel,k,l]
                                        dψIK_dy = dψ[m,k]*ψ1[n,l]*dξdy[iel,k,l] + ψ[m,k]*dψ1[n,l]*dηdy[iel,k,l]
                                        
                                        De[I,J] += ωkl*Jkle*(dψJK_dx+dψJK_dy)*(dψIK_dx+dψIK_dy)
...
function compute_matirx(params,connijk,iel,x,y)
                ...
                for m = 1:params.mesh.ngl # trail function
                    ...
                    for l = 1:params.mesh.ngl
                        for k = 1:params.mesh.ngl # quadrature points on each element
                            ...
                            De[I,J] = De[I,J] .+ ωkl*Jkle*(dψij_dx*dψmn_dx + dψij_dy*dψmn_dy)
    ...
    # Build right hand side
                ...
                for m = 1:params.mesh.ngl # trail function
                    ...
                    for l = 1:params.mesh.ngl
                        for k = 1:params.mesh.ngl # quadrature points on each element
                            ...
                            be[I] = be[I] .+ ωkl*Jkle*(params.uaux[index])*ψmn
            ...
            for n = 1:params.mesh.ngl
                for m = 1:params.mesh.ngl
                    ...
                    if(x[pos] >= 1.0 || x[pos] <= -1.0 || y[pos] >= 1.0 || y[pos] <= -1.0)                                                  
                        De[I,:] = zeros(TFloat,1,params.mesh.ngl*params.mesh.ngl)
                        De[I,I] = 1.0
                        be[I] = 0.0
                    end
...
function compute_coef(params,De,be)
    coef = zeros(TFloat,params.mesh.ngl*params.mesh.ngl,1)
    λ = 1e-9
    A = De + λ*I
    b = -be
    coef = A \ b   
